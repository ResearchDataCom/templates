...
A commit's scope **SHOULD** be the OpenTofu submodule name sans the
top-level prefix or any suffixes.  Changes covering multiple scopes or
changes not specific to one scope **MUST NOT** specify a scope,
including changes instigated by code in top-level resource
definitions.
